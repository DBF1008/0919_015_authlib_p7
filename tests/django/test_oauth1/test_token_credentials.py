...
def test_invalid_token_request_parameters(factory, server):
    url = "/oauth/token"
    ...
    # case 2
    request = factory.post(url, data={"oauth_consumer_key": "a"})
    resp = server.create_token_response(request)
    data = decode_response(resp.content)
    assert data["error"] == "invalid_client"

    # case 3
    request = factory.post(url, data={"oauth_consumer_key": "client"})
    resp = server.create_token_response(request)
    data = decode_response(resp.content)
    assert data["error"] == "missing_required_parameter"
...
def test_duplicated_oauth_parameters(factory, server):
...
def test_plaintext_signature(factory, server):
    url = "/oauth/token"
    ...
    request = factory.post(url, HTTP_AUTHORIZATION=auth_header)
    resp = server.create_token_response(request)
    data = decode_response(resp.content)
...
def test_hmac_sha1_signature(factory, server):
    url = "/oauth/token"
...
def test_rsa_sha1_signature(factory, rsa_server):
    url = "/oauth/token"
    server = rsa_server
